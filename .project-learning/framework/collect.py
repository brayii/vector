...
def finish_task(task_id:str, data=None, db_path=DB_PATH):
    ...
    try:
        ...
        if task["status"]=="completed": db.rollback(); return {"changed":False,"status":"completed"}
        ...
        db.execute("INSERT INTO outcomes VALUES(?,?,?,?,?,?,?)",(make_id("out"),task_id,data.get("result",""),data.get("success",True),json.dumps(data.get("metrics",{})),data.get("limitations",""),now())); db.execute("UPDATE tasks SET status='completed',verification_summary=?,duration_ms=? WHERE task_id=?",(data.get("verification",""),data.get("duration_ms"),task_id)); db.commit(); return {"changed":True,"status":"completed"}
